<template>
  <v-container>
    <h1>Video List</h1>
    <v-btn to="/admin/videos/add">Add Video</v-btn>
    ...
    <div v-for="video in videos" :key="video._id" class="flex-table">
      ...
      <div class="actions">
        <router-link class="link" :to="`videos/${video._id}`">Show</router-link>
        <router-link class="link" :to="`videos/${video._id}/edit`"
          >Edit</router-link
        >
...
export default {
  ...
  methods: {
    ...mapActions({
      deleteVideo: "videoModule/deleteVideo",
    }),
    removeVideo(video) {
